...
'''
    (data structure of state diagram: state, input, callback, new state)
    {'A': ['b_pressed', highlightB, 'B'], 'B': ['a_pressed', highlightA, 'A']}
    
    (state engine)
    while True:
        # get event from event queue
        if current_state == state AND event == input:
            callback()
            current_state = new state
            
    # ways to use data structure
    list - a heterogeneous, mutable data type that stores an order sequence of things
    Tuple {1, 2, 3, (4, 5)}
    Dictionary {'key': 132, 'key2': 321}
    Set {1, 2, 3, 5, 4}
'''
##############################
...
class StateMachine(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.q.empty():
                e = self.q.get()
                for c in self.states:  # this is where inefficiency come in
    ...
    def update_states_map(self, current_state):
        ...
        for state_name, val in self.states.items():  # step into state machine
            if state_name not in drawn_states:
                ...
                # now do the work: changing the color of state display
...
def main():
    ...
    # create two instances to run
    sm = StateMachine('Parking Ticket FSM', q)
    tm = TicketMachine(gui_handle, sm)

    # tm run by itself in main thread, but we need to give tm a thread
